    ...
    while (frames_written < total_frames) {
        ...
        for (uint32_t i = 0; i < frames_this_chunk; i++) {
            ...
            for (uint8_t ch = 0; ch < effective_channels; ch++) {
                ((int16_t *)buf)[i * effective_channels + ch] = sample;
            }
            phase += phase_step;
        ...
        if (esp_codec_dev_write(dac->codec_dev, buf,
                                (int)(frames_this_chunk * effective_channels * sizeof(int16_t))) != ESP_CODEC_DEV_OK) {
            free(buf);
